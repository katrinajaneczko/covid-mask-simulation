function janeczko_final_project
%JANECZKO_FINAL_PROJECT
% This code simulates the effect of mask type on the spread of COVID-19 on 
% a college campus. To run, choose from the experimental distributions
% below (q_percentages) which dictates reasonable percentages of all agents
% wearing each mask type (no mask, cloth mask, surgical mask, or
% respirator). Beginning with an initial number of N=395 agents (a
% scaled-down version of Temple's student population) and an initial
% positivity rate of i=0.019, agents are randomly placed on a 10x10 grid
% with their assigned attribute of mask type (or infected). They will
% wander the grid for about the duration of a semester where each timestep
% is an hour, and when another agent is present in a grid square with an 
% infected agent, they have a certain probability of becoming infected that
% is determined by the base probability modified by the probability dictated
% by their mask type. Once an agent is infected for 6 days, they are
% removed (to quarantine) and are reintroduced as immune (regardless of
% mask type) to the population. With each time step, with a small
% probability, a new infectious agent may be introduced into the
% population. The code plots a 10x10 grid on the left representing the campus
% where each student is a colored dot representing mask type of infection 
% status, and a line graph on the right showing the proportion of infected
% students over time (with a bar for current population size).
%
% Katrina Janeczko 05/2022

% Experiments: choose initial percentages of population that uses each kind of mask (or lack thereof)
%q_percentages = [1, 1, 1, 1]; % no masks at all
%q_percentages = [0.5, 0.75, 0.9, 1]; % no mask mandate
q_percentages = [0.01, 0.7, 0.9, 1]; % regular mask mandate
%q_percentages = [0.01, 0.05, 0.9, 1]; % surgical mask mandate
%q_percentages = [0.01, 0.05, 0.1, 1]; % respirator mask mandate
%q_percentages = [0, 0, 0, 1]; % only respirators

% Agent Parameters
N = 395; % initial number of agents (39,581 students attend Temple)
i = 0.019; % initial positivity rate
new = 0.01; % probability to add new infected agent every hour
v = .025; % speed of agents (distance traveled per step)
...
q_base = r0 / (avg_pop_density * avg_cells_visited * max_infection); % max_infection is amount of opportunities to infect
q_modifier = [1, 0.44, 0.34, 0.17, 0]; % no mask, cloth mask, surgical mask, respirator, immune-regardless of mask
...
for j = 1:N % for all agents
    for k = 1:4 % for the 4 different mask types
        if tempM(j) <= q_percentages(k) % determine which agents have which masks based on the wearing percentages
            M(j) = k; 
            break
        end
    end
...
for j = 1:ns % loop over steps
    ...
    for k = 1:size(cells_with_multiple_agents,1) % for every cell with adequate number of agents
        ...
        if covid_present % if COVID is present in this cell
            for l = 1:size(agents_in_this_cell) % find the agents without COVID, and with a probability of the base COVID probability times the adjusted odds ratio 
                if ~I(agents_in_this_cell(l))>=1 && rand<(q_base * q_modifier(M(agents_in_this_cell(l)))) % of the specific type of mask they were using
    ...
    plot(X(ind,1),X(ind,2), '.','markersize',12,'color',[0.6 0.2 0.8], 'DisplayName', sprintf('No Mask (%d)',nnz(ind)));    
    ind = ~I>=1 & M==2; % uninfected agents with cloth mask
    plot(X(ind,1),X(ind,2),'.','markersize',12,'color',[0 0.30 .70], 'DisplayName', sprintf('Cloth Mask (%d)',nnz(ind)));
    ind = ~I>=1 & M==3; % uninfected agents with surgical mask
    plot(X(ind,1),X(ind,2),'.','markersize',12,'color',[0 .75 .65], 'DisplayName', sprintf('Surgical Mask (%d)',nnz(ind)));
    ind = ~I>=1 & M==4; % uninfected agents with respirator mask
    plot(X(ind,1),X(ind,2),'.','markersize',12,'color',[0.02 1 0.2], 'DisplayName', sprintf('Respirator Mask (%d)',nnz(ind)));
    ...
    title('Spread of COVID on college campus')
    ...
    xlabel('Hours'), ylabel('Infected Students')
